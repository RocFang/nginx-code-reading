...
/*
ports是一个数组，数组元素的类型为ngx_rtmp_conf_port_t
ngx_rtmp_conf_port_t结构存放了一个端口和同时监听该端口的多个ip组成的数组
*/
...
// pm=Protocol Control Messages
    static size_t               pm_events[] = {
		//对应rtmpsecp的5.4：protocal control message
		//Set Chunk Size (1)
...
//重要技巧:在http中，并未使用ls的这个servers成员
            ls->servers = mport;
