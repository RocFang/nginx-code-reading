...
/*
out_queue默认为256
除了分配一个ngx_rtmp_session的内存外，还额外分配了out_queue(默认为256)个ngx_chain_t类型的指针
*/
...
/*
将ngx_connection_t的data成员赋值为ngx_rtmp_session_t,跟http中，ngx_connection_t的data成员为ngx_http_request_t一样
*/
    c->data = s;
    s->connection = c;
...
/*
为模块上下文分配内存。模块上下文类似于http中的ngx_http_set_ctx和ngx_http_get_module_ctx
*/
...
//out_cork为out_queue/8，所以默认为32
    s->out_cork = cscf->out_cork;
//max_streams默认为32，分配32个ngx_rtmp_stream_t结构体
...
/*
执行ngx_rtmp_limit_connect，判断连接数是否超过配置额度。默认为不限制，直接退出
*/
