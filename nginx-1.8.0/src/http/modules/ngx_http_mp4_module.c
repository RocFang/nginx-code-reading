...
//¶ÔÓ¦mp4_buffer_sizeºÍmp4_max_buffer_sizeÁ½¸öÅäÖÃÏî
typedef struct {
    size_t                buffer_size;
...
¿ÉÒÔ¿´µ½stsc boxÀïÃ¿¸öentry½á¹¹Ìå¶¼´æÓĞÈıÏîÊı¾İ£¬ËüÃÇµÄÒâË¼ÊÇ£º¡°´Ófirst_chunkÕâ¸öchunkĞòºÅ¿ªÊ¼£¬Ã¿¸öchunk¶¼ÓĞsamples_per_chunk¸öÊıµÄsample£¬ 
¶øÇÒÃ¿¸ösample¶¼¿ÉÒÔÍ¨¹ısample_description_indexÕâ¸öË÷Òı£¬ÔÚstsd boxÖĞÕÒµ½ÃèÊöĞÅÏ¢¡±¡£Ò²¾ÍÊÇËµ£¬Ã¿¸öentry½á¹¹ÌåÃèÊöµÄÊÇÒ»×échunk£¬ 
ËüÃÇÓĞÏàÍ¬µÄÌØµã£¬ÄÇ¾ÍÊÇÃ¿¸öchunk°üº¬samples_per_chunk¸ösample£¬ºÃ£¬ÄÇÄãÒªÎÊ£¬Õâ×éÏàÍ¬ÌØµãµÄchunkÓĞ¶àÉÙ¸ö£¿ 
ÇëÍ¨¹ıÏÂÒ»¸öentry½á¹¹ÌåÀ´ÍÆËã£¬ÓÃÏÂÒ»¸öentryµÄfirst_chunk¼õÈ¥±¾´ÎµÄfirst_chunk£¬¾ÍµÃµ½ÁËÕâ×échunkµÄ¸öÊı¡£ 
×îºóÒ»¸öentry½á¹¹ÌåÔò±íÃ÷´Ó¸Ãfirst_chunkµ½×îºóÒ»¸öchunk£¬Ã¿¸öchunk¶¼ÓĞsampls_per_chunk¸ösample¡£ºÜŞÖ¿Ú°É£¬²»¹ı£¬¾ÍÊÇÕâ¸öÒâË¼:)¡£ 
ÓÉÓÚÕâÖÖËã·¨ÎŞ·¨µÃÖªÎÄ¼şËùÓĞchunkµÄ¸öÊı£¬ËùÒÔÄã±ØĞë½èÖúÓÚstco»òco64
*/
typedef struct {
// FirstChunk ×Ö¶Î
    u_char                chunk[4];
...
//´ò¿ªµÄmp4ÎÄ¼şÃèÊö·û
	...
	// start,ÇëÇó²ÎÊı£¬µ¥Î»ÎªºÁÃë,ÀıÈç£¬ÇëÇó²ÎÊıÎªstart=2,ÔòÕâÀïµÄstartÎª2000
    ngx_uint_t            start;
...
/*
×îÍâ²ãµÄÈı¸öºĞ×ÓµÄ´¦Àí
ftyp: File type and compatibility¡£ÔÚÎÄ¼şµÄ¿ªÊ¼Î»ÖÃ£¬ÃèÊöµÄÎÄ¼şµÄ°æ±¾¡¢¼æÈİĞ­ÒéµÈ
moov: Container for structural metadata¡£Õâ¸öboxÖĞ²»°üº¬¾ßÌåÃ½ÌåÊı¾İ£¬µ«°üº¬±¾ÎÄ¼şÖĞËùÓĞÃ½ÌåÊı¾İµÄºê¹ÛÃèÊöĞÅÏ¢£¬moov boxÏÂÓĞmvhdºÍtrak box
mdat: Media data container¡£Êµ¼ÊÃ½ÌåÊı¾İ¡£ÎÒÃÇ×îÖÕ½âÂë²¥·ÅµÄÊı¾İ¶¼ÔÚÕâÀïÃæ
*/
...
/*
moovµÄÁ½ÖÖ×ÓºĞ×Ó£¬mvhdºÍtrak
mvhd: ±»moovºĞ×Ó°üº¬.playback information. mvhd box should be placed first in its container¡£mvhdÖĞ¼ÇÂ¼ÁË´´½¨Ê±¼ä¡¢ĞŞ¸ÄÊ±¼ä¡¢Ê±¼ä¶ÈÁ¿±ê³ß¡¢¿É²¥·ÅÊ±³¤µÈĞÅÏ¢
trak: ±»moovºĞ×Ó°üº¬.Each Track(trak) box corresponds to a individual media track witin the mp4 file and contains boxes that further define
      the properities of the media track.trakÖĞµÄÒ»ÏµÁĞ×ÓboxÃèÊöÁËÃ¿¸öÃ½Ìå¹ìµÀµÄ¾ßÌåĞÅÏ¢¡£
*/
...
/*
stblºĞ×ÓµÄ¼¸ÖÖ×ÓºĞ×Ó,²Î¿¼http://blog.csdn.net/yu_yuan_1314/article/details/9078287
stsd:sample descriptions (codec types, initialization etc.)
stts:(decoding) time-to-sample
stss:sync sample table (random access points)
ctts:(composition) time to sampleÊ±¼äºÏ³ÉÆ«ÒÆ±í
stsc:sample-to-chunk, partial data-offset information,ChunkÖĞµÄSampleĞÅÏ¢±í
stsz:sample sizes (framing)Ö¸¶¨ÁËÃ¿¸ösampleµÄsize
stco:chunk offset, partial data-offset information.ChunkµÄÆ«ÒÆÁ¿±ístco/co64,Ö¸¶¨ÁËÃ¿¸öchunkÔÚÎÄ¼şÖĞµÄÎ»ÖÃ
...
//clcf->read_aheadÓÉhttp://nginx.org/en/docs/http/ngx_http_core_module.html#read_ahead ÅäÖÃ£¬ÔÚlinuxÆ½Ì¨±»ºöÂÔ
...
//ÔÊĞírangeÇëÇó
    ...
    if (r->args.len) {
//½âÎöstart²ÎÊı,µ¥Î»ÎªÃë£¬µ«½âÎöºó×ª»»ÎªºÁÃë
...
//½âÎöend²ÎÊı£¬µ¥Î»ÎªÃë,µ«½âÎöºó×ª»¯ÎªºÁÃë
...
/*
ÔÚ´Ëº¯Êıµ÷ÓÃÖ®Ç°£¬mp4½á¹¹½öÓĞÈçÏÂ³õÊ¼»¯²Ù×÷:
mp4->file.fd = of.fd;
mp4->file.name = path;
mp4->file.log = r->connection->log;
mp4->end = of.size;
...
// ´«ÈëµÄmp4->end ÎªÎÄ¼şµÄ´óĞ¡£¬¶ø·ÇÇëÇó²ÎÊıÖĞµ¥Î»ÎªÃëµÄend¡£
//µ÷ÓÃngx_http_mp4_read_atom½âÎömp4×îÍâ²ãµÄÈı¸öºĞ×Ó:ftyp,moov,mdat
...
/*
µ±Ìí¼Ósamples µ½media Ê±£¬ÓÃchunks ×éÖ¯ÕâĞ©sample£¬ÕâÑù¿ÉÒÔ·½±ãÓÅ»¯Êı¾İ»ñÈ¡¡£
Ò»¸ötrunk °üº¬Ò»¸ö»ò¶à¸ösample£¬chunkµÄ³¤¶È¿ÉÒÔ²»Í¬£¬chunk ÄÚµÄsample µÄ³¤¶ÈÒ²¿ÉÒÔ²»Í¬¡
sample-to-chunk atom ´æ´¢sample Óëchunk µÄÓ³Éä¹ØÏµ¡£Sample-to-chunk atoms µÄÀàĞÍÊÇ'stsc'¡£
ËüÒ²ÓĞÒ»¸ö±íÀ´Ó³Éäsample ºÍtrunk Ö®¼äµÄ¹ØÏµ£¬²é¿´ÕâÕÅ±í£¬¾Í¿ÉÒÔÕÒµ½°üº¬Ö¸¶¨sample µÄtrunk£¬´Ó¶øÕÒµ½Õâ¸ösample¡£
...
/*
Sample Size table¡£Õâ¸ö±í°üº¬ÁËÃ¿¸ösample µÄ³¤¶È£¬ÕÒµ½sample µÄĞòºÅ£¬¾Í¿ÉÒÔÕÒµ½¶ÔÓ¦sample µÄ³¤¶ÈÁË¡£
*/
...
/*
Chunk offset atoms ¶¨ÒåÁËÃ¿¸ötrunk ÔÚÃ½ÌåÁ÷ÖĞµÄÎ»ÖÃ£¬ËüµÄÀàĞÍÊÇ'stco'¡£Î»ÖÃÓĞÁ½ÖÖ¿ÉÄÜ£¬32 Î»µÄºÍ64 Î»µÄ£¬ºóÕß¶Ô·Ç³£´óµÄµçÓ°ºÜÓĞÓÃ¡£
ÔÚÒ»¸ö±íÖĞÖ»»áÓĞÒ»ÖÖ¿ÉÄÜ£¬Õâ¸öÎ»ÖÃÊÇÔÚÕû¸öÎÄ¼şÖĞµÄ£¬¶ø²»ÊÇÔÚÈÎºÎatom ÖĞµÄ£¬ÕâÑù×ö¾Í¿ÉÒÔÖ±½ÓÔÚÎÄ¼şÖĞÕÒµ½Ã½ÌåÊı¾İ£¬¶ø²»ÓÃ½âÊÍatom¡£
ĞèÒª×¢ÒâµÄÊÇÒ»µ©Ç°ÃæµÄatom ÓĞÁËÈÎºÎ¸Ä±ä£¬ÕâÕÅ±í¶¼ÒªÖØĞÂ½¨Á¢£¬ÒòÎªÎ»ÖÃĞÅÏ¢ÒÑ¾­¸Ä±äÁË¡£
*/
...
/*
Ã¿¸öboxÓÉheaderºÍdata×é³É£¬headerÖĞ°üÀ¨ÁËÕû¸öboxµÄ³¤¶ÈsizeºÍÀàĞÍtype(¼´ÏÂÃæÊı¾İ½á¹¹ÖĞµÄname),
µ±size==0Ê±£¬´ú±íÕâÊÇÎÄ¼şÖĞ×îºóÒ»¸öbox£¬µ±size==1Ê±£¬ÒâÎ¶×Åbox³¤¶ÈĞèÒªÓÃ¸ü¶àbit±íÊ¾£¬ËùÒÔÔÚºóÃæ
»áÓĞÒ»¸ö64bitµÄlargesizeÀ´ÃèÊöboxµÄ³¤¶È¡£

ËùÒÔÕâÀï¶¨ÒåÁËÁ½ÖÖbox header£¬¼´ngx_mp4_atom_header_tºÍngx_mp4_atom_header64_t
*/
typedef struct {
    u_char    size[4];
    u_char    name[4];
...
//end±íÊ¾±¾´Î½âÎöºóÎ²²¿ÔÚÕû¸ömp4 ÎÄ¼şÖĞµÄÆ«ÒÆ
//mp4->offset±íÊ¾±¾´Î½âÎöÇ°µ±Ç°Î»ÖÃÔÚÕû¸ömp4ÎÄ¼şÖĞµÄÆ«ÒÆ
//atom_data_size±íÊ¾±¾´Î½âÎöĞèÒª½âÎöµÄ´óĞ¡
    end = mp4->offset + atom_data_size;

    //whileÎªtrueÊ±±íÊ¾±¾´Î½âÎö»¹Ã»Íê³É
    while (mp4->offset < end) {

        //µ÷ÓÃngx_http_mp4_read¼ì²éµ±Ç°bufferÊÇ·ñ¹»´ó£¬Èç¹û²»¹»£¬ÔòÖØĞÂ·ÖÅäbuffer
        if (ngx_http_mp4_read(mp4, sizeof(uint32_t)) != NGX_OK) {
            return NGX_ERROR;
        }

        atom_header = mp4->buffer_pos;
		//½âÎöbox header,´ÓÖĞ»ñÈ¡boxµÄ´óĞ¡
...
//get box type
        atom_header = mp4->buffer_pos;
//atom_name¾ÍÊÇbox header½á¹¹ÖĞµÄBoxType
...
//µ±Ç°bufferÊ£ÏÂµÄ¿Õ¼äÒÑ¾­¹»×°ÏÂ¼´½«¶ÁÈ¡µÄsizeÁË£¬¾ÍÖ±½ÓÍË³ö¸Ãº¯Êı£¬²»ÓÃ½øĞĞÏÂÃæµÄÔÙ·ÖÅä
    if (mp4->buffer_pos + size <= mp4->buffer_end) {
        return NGX_OK;
    }

//mp4->buffer_size³õÊ¼ÖµÎªÅäÖÃÎÄ¼şÖĞÅäÖÃµÄbuffer_size
//mp4->offset±íÊ¾µ±Ç°ÒÑ¾­½âÎöµ½µÄÎ»ÖÃÔÚmp4ÎÄ¼şÖĞµÄÆ«ÒÆ
...
//´Ómp4ÎÄ¼şµÄµ±Ç°ÒÑ½âÎöÎ»ÖÃmp4->offset¿ªÊ¼£¬¶ÁÈ¡mp4->buffer_sizeµÄÄÚÈİµ½¸Õ·ÖÅäµÄĞÂbufferÖĞ¡£ngx_read_file»áÒıÆğmp4->file->offsetµÄÔö¼Ó
...
/*
×îÖØÒªµÄ½âÎö¹ı³Ì:½âÎömetadata,¼´moovºĞ×Ó
*/
static ngx_int_t
...
//µ÷ÓÃngx_http_mp4_read_atom½âÎömoovºĞ×Ó£¬Ö÷ÒªÊÇmvhdºÍtrakºĞ×Ó
...
/*
mvhdºĞ×Ó:
Èç¹ûversionÎª0£¬ÔòÊ¹ÓÃngx_mp4_mvhd_atom_t£¬Èç¹ûversionÎª1£¬ÔòÊ¹ÓÃngx_mp4_mvhd64_atom_t
*/
typedef struct {
//size ºÍ name Îªheader
    u_char    size[4];
    u_char    name[4];
//version Îª0»ò1
    u_char    version[1];
//flagÎª24Î»£¬±£ÁôÎª0
    u_char    flags[3];
//Èç¹ûversionÎª0£¬ÔòCreationTimeºÍModificationTime¾ùÎª32Î»£¬·ñÔòÎª64Î»
    u_char    creation_time[4];
    u_char    modification_time[4];
//TimeScale:32Î»
    u_char    timescale[4];
//Èç¹ûversionÎª0£¬ÔòDurationÎª32Î»£¬·ñÔòÎª64Î»
    u_char    duration[4];
    u_char    rate[4];
    u_char    volume[2];
//80¸öÎ»µÄ±£Áô×Ö
...
//Reserved: 32*6=192Î»£¬ËùÒÔÏÂÃæÒ»¹²ÓĞ6*4*8Î»£¬¾ÍÊÇÀûÓÃÁËÕâ192Î»µÄ±£Áô×Ö
...
//mvhd boxµÄ´¦Àíº¯Êı,Ö÷Òª¸ºÔğ¸üĞÂduration
static ngx_int_t
...
*/
    ...
    // mp4->startµ¥Î»ÎªºÁÃë£¬ËùÒÔÒª³ıÒÔ1000×ª»»³ÉÃë¡£start_time½«ÇëÇó²ÎÊıÖĞµ¥Î»ÎªÃëµÄstart×ª»»Îªmp4ÖĞµÄtime unit.
...
//mp4->lengthµÄµ¥Î»ÎªºÁÃë£¬ÊÇ²ÎÊıÖĞendÓëstartµÄ²îÖµ×ª»»ÎªºÁÃë
    if (mp4->length) {
		//length_time ±íÊ¾ÇëÇó²ÎÊıÖĞendÓëstartÖ®²î£¬×ª»»Îªmp4µÄtime unitºóµÄ³¤¶È
...
//µ÷ÓÃngx_http_mp4_read_atom½âÎötrakºĞ×Ó£¬Ö÷ÒªÊÇtkhdºÍmediaºĞ×Ó
...
/*
µ÷ÓÃngx_http_mp4_read_atom½âÎömdiaºĞ×Ó£¬Ö÷ÒªÊÇmdhd,hdlr,minfÈı¸ö×ÓºĞ×Ó
*/
...
//µ÷ÓÃngx_http_mp4_read_atom½âÎöminfºĞ×Ó£¬Ö÷ÒªÊÇvmhd,smhd,dinf,stblËÄ¸ö×ÓºĞ×Ó
...
/*
µ÷ÓÃngx_http_mp4_read_atom½âÎöstblºĞ×Ó£¬Ö÷ÒªÊÇ
"stsd"
"stts",
"stss", 
"ctts", 
...
typedef struct {
	// sizeºÍnameÎªheader,·Ö±ğ±íÕ÷sizeºÍtypeÊôĞÔ
    u_char    size[4];
    u_char    name[4];
	// versionºÍflag¾ù±£ÁôÎª0
    u_char    version[1];
    u_char    flags[3];
    // Count
    u_char    entries[4];
...
typedef struct {
	...
	//24Î»Flags,±£ÁôÎ»£¬Îª0
    u_char    flags[3];
	//Count,32Î»:The number of STTSRECORD entries
    u_char    entries[4];
} ngx_mp4_stts_atom_t;

typedef struct {
	//SampleCount,Îª32Î»:The number of consecutive samples that this STTSRECORD applies to
    u_char    count[4];
	//SampleDelta,Îª32Î»:Sample duration in TimeScale units defined in the mdhd box
    u_char    duration[4];
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    start_sample = 0;
	// entryÎªSTTSRECORD entriesÊı×éµÄÆğÊ¼µØÖ·
...
sync sample atomÈ·¶¨mediaÖĞµÄ¹Ø¼üÖ¡¡£¶ÔÓÚÑ¹ËõµÄÃ½Ìå£¬¹Ø¼üÖ¡ÊÇÒ»ÏµÁĞÑ¹ËõĞòÁĞµÄ¿ªÊ¼Ö¡£¬ËüµÄ½âÑ¹ËõÊÇ²»ÒÀÀµÓÚÒÔÇ°µÄÖ¡¡£ºóĞøÖ¡µÄ½âÑ¹ËõÒÀÀµÓÚÕâ¸ö¹Ø¼üÖ¡¡£
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
		// start_sample ÔÚngx_http_mp4_update_stts_atomÖĞµÄngx_http_mp4_crop_stts_dataÖĞ¸üĞÂ
        ...
        //end_sampleÔÚngx_http_mp4_update_stts_atomÖĞµÄngx_http_mp4_crop_stts_dataÖĞ¸üĞÂ
...
/*
Ê±¼äºÏ³ÉÆ«ÒÆ±íctts
ctts£ºComposition Offset Atom¡£Ã¿Ò»¸öÊÓÆµsample¶¼ÓĞÒ»¸ö½âÂëË³ĞòºÍÒ»¸öÏÔÊ¾Ë³Ğò¡£¶ÔÓÚÒ»¸ösampleÀ´Ëµ£¬½âÂëË³ĞòºÍÏÔÊ¾Ë³Ğò¿ÉÄÜ²»Ò»ÖÂ£¬±ÈÈçH.264¸ñÊ½£¬ 
Òò´Ë£¬Composition Offset Atom¾ÍÊÇÔÚÕâÖÖÇé¿öÏÂ±»Ê¹ÓÃµÄ¡£
£¨1£©Èç¹û½âÂëË³ĞòºÍÏÔÊ¾Ë³ĞòÊÇÒ»ÖÂµÄ£¬Composition Offset Atom¾Í²»»á³öÏÖ¡£Time-To-Sample Atoms¼ÈÌá¹©ÁË½âÂëË³ĞòÒ²Ìá¹©ÁËÏÔÊ¾Ë³Ğò£¬ 
²¢ÄÜ¹»¼ÆËã³öÃ¿¸ösampleµÄ¿ªÊ¼Ê±¼äºÍ½áÊøÊ±¼ä¡£
£¨2£©Èç¹û½âÂëË³ĞòºÍÏÔÊ¾Ë³Ğò²»Ò»ÖÂ£¬ÄÇÃ´Time-To-Sample Atoms¼ÈÌá¹©½âÂëË³Ğò£¬Composition Offset AtomÔòÍ¨¹ı²îÖµµÄĞÎÊ½À´Ìá¹©ÏÔÊ¾Ê±¼ä¡£
Composition Offset AtomÌá¹©ÁËÒ»¸ö´Ó½âÂëÊ±¼äµ½ÏÔÊ¾Ê±¼äµÄsampleÒ»¶ÔÒ»µÄÓ³Éä£¬¾ßÓĞÈçÏÂµÄÓ³Éä¹ØÏµ£º
CT(n) = DT(n) + CTTS(n)
ÆäÖĞ£¬CTTS(n)ÊÇsample nÔÚtableÖĞµÄentry£¨ÕâÀï¼ÙÉèÒ»¸öentryÖ»¶ÔÓ¦Ò»¸ösample£©¿ÉÒÔÊÇÕıÖµÒ²¿ÉÊÇ¸ºÖµ£»DT(n)ÊÇsample nµÄ½âÂëÊ±¼ä£¬ 
Í¨¹ıTime-To-Sample Atoms¼ÆËã»ñµÃ£»CT(n)±ãÊÇsample nµÄÏÔÊ¾Ê±¼ä¡£
*/
static void
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
		// start_sample ÔÚngx_http_mp4_update_stts_atomÖĞµÄngx_http_mp4_crop_stts_dataÖĞ±»È·¶¨
        start_sample = trak->start_sample + 1;
        ...
        //end_sampleÔÚngx_http_mp4_update_stts_atomÖĞµÄngx_http_mp4_crop_stts_dataÖĞ±»È·¶¨
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
		// start_sample ÔÚngx_http_mp4_crop_stts_dataÖĞ¸üĞÂ
        ...
        // end_sampleÔÚngx_http_mp4_crop_stts_dataÖĞ¸üĞÂ
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (data) {
        ...
        // trak->start_sampleºÍtrak.end_sample¾ùÔÚngx_http_mp4_crop_stts_dataÖĞ¸üĞÂ
