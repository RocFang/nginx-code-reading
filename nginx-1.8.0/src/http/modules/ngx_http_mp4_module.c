...
//ÓÃÔÚngx_http_mp4_trak_tÖÐ£¬ngx_chain_t out[NGX_HTTP_MP4_LAST_ATOM + 1];
...
¿ÉÒÔ¿´µ½stsc boxÀïÃ¿¸öentry½á¹¹Ìå¶¼´æÓÐÈýÏîÊý¾Ý£¬ËüÃÇµÄÒâË¼ÊÇ£º¡°´Ófirst_chunkÕâ¸öchunkÐòºÅ¿ªÊ¼£¬Ã¿¸öchunk¶¼ÓÐsamples_per_chunk¸öÊýµÄsample£¬ 
¶øÇÒÃ¿¸ösample¶¼¿ÉÒÔÍ¨¹ýsample_description_indexÕâ¸öË÷Òý£¬ÔÚstsd boxÖÐÕÒµ½ÃèÊöÐÅÏ¢¡±¡£Ò²¾ÍÊÇËµ£¬Ã¿¸öentry½á¹¹ÌåÃèÊöµÄÊÇÒ»×échunk£¬ 
ËüÃÇÓÐÏàÍ¬µÄÌØµã£¬ÄÇ¾ÍÊÇÃ¿¸öchunk°üº¬samples_per_chunk¸ösample£¬ºÃ£¬ÄÇÄãÒªÎÊ£¬Õâ×éÏàÍ¬ÌØµãµÄchunkÓÐ¶àÉÙ¸ö£¿ 
ÇëÍ¨¹ýÏÂÒ»¸öentry½á¹¹ÌåÀ´ÍÆËã£¬ÓÃÏÂÒ»¸öentryµÄfirst_chunk¼õÈ¥±¾´ÎµÄfirst_chunk£¬¾ÍµÃµ½ÁËÕâ×échunkµÄ¸öÊý¡£ 
×îºóÒ»¸öentry½á¹¹ÌåÔò±íÃ÷´Ó¸Ãfirst_chunkµ½×îºóÒ»¸öchunk£¬Ã¿¸öchunk¶¼ÓÐsampls_per_chunk¸ösample¡£ºÜÞÖ¿Ú°É£¬²»¹ý£¬¾ÍÊÇÕâ¸öÒâË¼:)¡£ 
ÓÉÓÚÕâÖÖËã·¨ÎÞ·¨µÃÖªÎÄ¼þËùÓÐchunkµÄ¸öÊý£¬ËùÒÔÄã±ØÐë½èÖúÓÚstco»òco64
*/
typedef struct {
// FirstChunk ×Ö¶Î
    u_char                chunk[4];
...
typedef struct {
    uint32_t              timescale;
	//¸Ãtrak stts±íÖÐentryµÄÊýÄ¿,stts±íÊ¾time to sample
    uint32_t              time_to_sample_entries;
	//¸Ãtrak stsc±íÖÐentryµÄÊýÄ¿£¬stsc±íÊ¾ sample to chunk
    uint32_t              sample_to_chunk_entries;
    uint32_t              sync_samples_entries;
	//¸ÃtrakÖÐctts±íÖÐentryµÄÊýÄ¿,ctts±íÊ¾Composition Time to Sample box
    uint32_t              composition_offset_entries;
	//¸ÃtrakÖÐstsz±íÖÐentryµÄÊýÄ¿£¬stsz±íÊ¾sample to sample sizeµÄÓ³Éä
 ...
 /* Êý×éÖÐµÄÃ¿Ò»¸ö½ÚµãµÄbufÖ¸ÕëÖ¸ÏòÏÂÃæµÄ¸÷¸ö¶ÔÓ¦µÄngx_buf_t³ÉÔ±, Êý×é³ÉÔ±¹¹³ÉÒ»¸öµ¥Á´±í*/
...
//´ò¿ªµÄmp4ÎÄ¼þÃèÊö·û
    ...
    //¼ÇÂ¼Ã¿´Î½âÎöÇ°µ±Ç°Î»ÖÃÔÚÕû¸ömp4ÎÄ¼þÖÐµÄÆ«ÒÆ
    off_t                 offset;
	// endÎªmp4ÎÄ¼þ´óÐ¡£¬¶ø·ÇÇëÇó²ÎÊýÖÐµ¥Î»ÎªÃëµÄend
    off_t                 end;
    off_t                 content_length;
	// start,ÇëÇó²ÎÊý£¬µ¥Î»ÎªºÁÃë,ÀýÈç£¬ÇëÇó²ÎÊýÎªstart=2,ÔòÕâÀïµÄstartÎª2000
    ngx_uint_t            start;
	// lengthÎªÇëÇó²ÎÊýÖÐendÓëstartÖ®²î£¬µ¥Î»ÎªºÁÃë
    ngx_uint_t            length;
    uint32_t              timescale;
	// Ö¸Ïòµ±Ç°ÇëÇó
...
/*
×îÍâ²ãµÄÈý¸öºÐ×ÓµÄ´¦Àí
ftyp: File type and compatibility¡£ÔÚÎÄ¼þµÄ¿ªÊ¼Î»ÖÃ£¬ÃèÊöµÄÎÄ¼þµÄ°æ±¾¡¢¼æÈÝÐ­ÒéµÈ
moov: Container for structural metadata¡£Õâ¸öboxÖÐ²»°üº¬¾ßÌåÃ½ÌåÊý¾Ý£¬µ«°üº¬±¾ÎÄ¼þÖÐËùÓÐÃ½ÌåÊý¾ÝµÄºê¹ÛÃèÊöÐÅÏ¢£¬moov boxÏÂÓÐmvhdºÍtrak box
mdat: Media data container¡£Êµ¼ÊÃ½ÌåÊý¾Ý¡£ÎÒÃÇ×îÖÕ½âÂë²¥·ÅµÄÊý¾Ý¶¼ÔÚÕâÀïÃæ
*/
...
/*
moovµÄÁ½ÖÖ×ÓºÐ×Ó£¬mvhdºÍtrak
mvhd: ±»moovºÐ×Ó°üº¬.playback information. mvhd box should be placed first in its container¡£mvhdÖÐ¼ÇÂ¼ÁË´´½¨Ê±¼ä¡¢ÐÞ¸ÄÊ±¼ä¡¢Ê±¼ä¶ÈÁ¿±ê³ß¡¢¿É²¥·ÅÊ±³¤µÈÐÅÏ¢
trak: ±»moovºÐ×Ó°üº¬.Each Track(trak) box corresponds to a individual media track witin the mp4 file and contains boxes that further define
      the properities of the media track.trakÖÐµÄÒ»ÏµÁÐ×ÓboxÃèÊöÁËÃ¿¸öÃ½Ìå¹ìµÀµÄ¾ßÌåÐÅÏ¢¡£
*/
...
/*
stblºÐ×ÓµÄ¼¸ÖÖ×ÓºÐ×Ó,²Î¿¼http://blog.csdn.net/yu_yuan_1314/article/details/9078287
stsd:sample descriptions (codec types, initialization etc.)
stts:(decoding) time-to-sample
stss:sync sample table (random access points)
ctts:(composition) time to sampleÊ±¼äºÏ³ÉÆ«ÒÆ±í
stsc:sample-to-chunk, partial data-offset information,ChunkÖÐµÄSampleÐÅÏ¢±í
stsz:sample sizes (framing)Ö¸¶¨ÁËÃ¿¸ösampleµÄsize
stco:chunk offset, partial data-offset information.ChunkµÄÆ«ÒÆÁ¿±ístco/co64,Ö¸¶¨ÁËÃ¿¸öchunkÔÚÎÄ¼þÖÐµÄÎ»ÖÃ
...
//ÔÊÐírangeÇëÇó
    ...
    if (r->args.len) {
//½âÎöstart²ÎÊý,µ¥Î»ÎªÃë£¬µ«½âÎöºó×ª»»ÎªºÁÃë
...
//½âÎöend²ÎÊý£¬µ¥Î»ÎªÃë,µ«½âÎöºó×ª»¯ÎªºÁÃë
...
/*
ÔÚ´Ëº¯Êýµ÷ÓÃÖ®Ç°£¬mp4½á¹¹½öÓÐÈçÏÂ³õÊ¼»¯²Ù×÷:
mp4->file.fd = of.fd;
mp4->file.name = path;
mp4->file.log = r->connection->log;
mp4->end = of.size;
...
// ´«ÈëµÄmp4->end ÎªÎÄ¼þµÄ´óÐ¡£¬¶ø·ÇÇëÇó²ÎÊýÖÐµ¥Î»ÎªÃëµÄend¡£
//µ÷ÓÃngx_http_mp4_read_atom½âÎömp4×îÍâ²ãµÄÈý¸öºÐ×Ó:ftyp,moov,mdat
...
/*
Ã¿¸öboxÓÉheaderºÍdata×é³É£¬headerÖÐ°üÀ¨ÁËÕû¸öboxµÄ³¤¶ÈsizeºÍÀàÐÍtype(¼´ÏÂÃæÊý¾Ý½á¹¹ÖÐµÄname),
µ±size==0Ê±£¬´ú±íÕâÊÇÎÄ¼þÖÐ×îºóÒ»¸öbox£¬µ±size==1Ê±£¬ÒâÎ¶×Åbox³¤¶ÈÐèÒªÓÃ¸ü¶àbit±íÊ¾£¬ËùÒÔÔÚºóÃæ
»áÓÐÒ»¸ö64bitµÄlargesizeÀ´ÃèÊöboxµÄ³¤¶È¡£
...
//end±íÊ¾±¾´Î½âÎöºóÎ²²¿ÔÚÕû¸ömp4 ÎÄ¼þÖÐµÄÆ«ÒÆ
//mp4->offset±íÊ¾±¾´Î½âÎöÇ°µ±Ç°Î»ÖÃÔÚÕû¸ömp4ÎÄ¼þÖÐµÄÆ«ÒÆ
//atom_data_size±íÊ¾±¾´Î½âÎöÐèÒª½âÎöµÄ´óÐ¡
    end = mp4->offset + atom_data_size;

    //whileÎªtrueÊ±±íÊ¾±¾´Î½âÎö»¹Ã»Íê³É
    while (mp4->offset < end) {

        //µ÷ÓÃngx_http_mp4_read¼ì²éµ±Ç°bufferÊÇ·ñ¹»´ó£¬Èç¹û²»¹»£¬ÔòÖØÐÂ·ÖÅäbuffer
        if (ngx_http_mp4_read(mp4, sizeof(uint32_t)) != NGX_OK) {
            return NGX_ERROR;
        }

        atom_header = mp4->buffer_pos;
		//½âÎöbox header,´ÓÖÐ»ñÈ¡boxµÄ´óÐ¡
...
//get box type
        atom_header = mp4->buffer_pos;
//atom_name¾ÍÊÇbox header½á¹¹ÖÐµÄBoxType
...
//µ±Ç°bufferÊ£ÏÂµÄ¿Õ¼äÒÑ¾­¹»×°ÏÂ¼´½«¶ÁÈ¡µÄsizeÁË£¬¾ÍÖ±½ÓÍË³ö¸Ãº¯Êý£¬²»ÓÃ½øÐÐÏÂÃæµÄÔÙ·ÖÅä
    if (mp4->buffer_pos + size <= mp4->buffer_end) {
        return NGX_OK;
    }

//mp4->buffer_size³õÊ¼ÖµÎªÅäÖÃÎÄ¼þÖÐÅäÖÃµÄbuffer_size
//mp4->offset±íÊ¾µ±Ç°ÒÑ¾­½âÎöµ½µÄÎ»ÖÃÔÚmp4ÎÄ¼þÖÐµÄÆ«ÒÆ
...
//´Ómp4ÎÄ¼þµÄµ±Ç°ÒÑ½âÎöÎ»ÖÃmp4->offset¿ªÊ¼£¬¶ÁÈ¡mp4->buffer_sizeµÄÄÚÈÝµ½¸Õ·ÖÅäµÄÐÂbufferÖÐ¡£ngx_read_file»áÒýÆðmp4->file->offsetµÄÔö¼Ó
...
/*
×îÖØÒªµÄ½âÎö¹ý³Ì:½âÎömetadata,¼´moovºÐ×Ó
*/
static ngx_int_t
...
//µ÷ÓÃngx_http_mp4_read_atom½âÎömoovºÐ×Ó£¬Ö÷ÒªÊÇmvhdºÍtrakºÐ×Ó
...
//Reserved: 32*6=192Î»£¬ËùÒÔÏÂÃæÒ»¹²ÓÐ6*4*8Î»£¬¾ÍÊÇÀûÓÃÁËÕâ192Î»µÄ±£Áô×Ö
...
//mvhd boxµÄ´¦Àíº¯Êý,Ö÷Òª¸ºÔð¸üÐÂduration
static ngx_int_t
...
*/
    ...
    // mp4->startµ¥Î»ÎªºÁÃë£¬ËùÒÔÒª³ýÒÔ1000×ª»»³ÉÃë¡£start_time½«ÇëÇó²ÎÊýÖÐµ¥Î»ÎªÃëµÄstart×ª»»Îªmp4ÖÐµÄtime unit.
...
//mp4->lengthµÄµ¥Î»ÎªºÁÃë£¬ÊÇ²ÎÊýÖÐendÓëstartµÄ²îÖµ×ª»»ÎªºÁÃë
    if (mp4->length) {
		//length_time ±íÊ¾ÇëÇó²ÎÊýÖÐendÓëstartÖ®²î£¬×ª»»Îªmp4µÄtime unitºóµÄ³¤¶È
...
//µ÷ÓÃngx_http_mp4_read_atom½âÎötrakºÐ×Ó£¬Ö÷ÒªÊÇtkhdºÍmediaºÐ×Ó
...
/*
µ÷ÓÃngx_http_mp4_read_atom½âÎömdiaºÐ×Ó£¬Ö÷ÒªÊÇmdhd,hdlr,minfÈý¸ö×ÓºÐ×Ó
*/
...
//µ÷ÓÃngx_http_mp4_read_atom½âÎöminfºÐ×Ó£¬Ö÷ÒªÊÇvmhd,smhd,dinf,stblËÄ¸ö×ÓºÐ×Ó
...
/*
µ÷ÓÃngx_http_mp4_read_atom½âÎöstblºÐ×Ó£¬Ö÷ÒªÊÇ
"stsd"
"stts",
"stss", 
"ctts", 
...
typedef struct {
	// sizeºÍnameÎªheader,·Ö±ð±íÕ÷sizeºÍtypeÊôÐÔ
    u_char    size[4];
    u_char    name[4];
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
		// start_sample ÔÚngx_http_mp4_update_stts_atomÖÐµÄngx_http_mp4_crop_stts_dataÖÐ¸üÐÂ
        ...
        //end_sampleÔÚngx_http_mp4_update_stts_atomÖÐµÄngx_http_mp4_crop_stts_dataÖÐ¸üÐÂ
...
/*
Ê±¼äºÏ³ÉÆ«ÒÆ±íctts
ctts£ºComposition Offset Atom¡£Ã¿Ò»¸öÊÓÆµsample¶¼ÓÐÒ»¸ö½âÂëË³ÐòºÍÒ»¸öÏÔÊ¾Ë³Ðò¡£¶ÔÓÚÒ»¸ösampleÀ´Ëµ£¬½âÂëË³ÐòºÍÏÔÊ¾Ë³Ðò¿ÉÄÜ²»Ò»ÖÂ£¬±ÈÈçH.264¸ñÊ½£¬ 
Òò´Ë£¬Composition Offset Atom¾ÍÊÇÔÚÕâÖÖÇé¿öÏÂ±»Ê¹ÓÃµÄ¡£
£¨1£©Èç¹û½âÂëË³ÐòºÍÏÔÊ¾Ë³ÐòÊÇÒ»ÖÂµÄ£¬Composition Offset Atom¾Í²»»á³öÏÖ¡£Time-To-Sample Atoms¼ÈÌá¹©ÁË½âÂëË³ÐòÒ²Ìá¹©ÁËÏÔÊ¾Ë³Ðò£¬ 
²¢ÄÜ¹»¼ÆËã³öÃ¿¸ösampleµÄ¿ªÊ¼Ê±¼äºÍ½áÊøÊ±¼ä¡£
£¨2£©Èç¹û½âÂëË³ÐòºÍÏÔÊ¾Ë³Ðò²»Ò»ÖÂ£¬ÄÇÃ´Time-To-Sample Atoms¼ÈÌá¹©½âÂëË³Ðò£¬Composition Offset AtomÔòÍ¨¹ý²îÖµµÄÐÎÊ½À´Ìá¹©ÏÔÊ¾Ê±¼ä¡£
Composition Offset AtomÌá¹©ÁËÒ»¸ö´Ó½âÂëÊ±¼äµ½ÏÔÊ¾Ê±¼äµÄsampleÒ»¶ÔÒ»µÄÓ³Éä£¬¾ßÓÐÈçÏÂµÄÓ³Éä¹ØÏµ£º
CT(n) = DT(n) + CTTS(n)
ÆäÖÐ£¬CTTS(n)ÊÇsample nÔÚtableÖÐµÄentry£¨ÕâÀï¼ÙÉèÒ»¸öentryÖ»¶ÔÓ¦Ò»¸ösample£©¿ÉÒÔÊÇÕýÖµÒ²¿ÉÊÇ¸ºÖµ£»DT(n)ÊÇsample nµÄ½âÂëÊ±¼ä£¬ 
Í¨¹ýTime-To-Sample Atoms¼ÆËã»ñµÃ£»CT(n)±ãÊÇsample nµÄÏÔÊ¾Ê±¼ä¡£
*/
static void
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
		// start_sample ÔÚngx_http_mp4_update_stts_atomÖÐµÄngx_http_mp4_crop_stts_dataÖÐ±»È·¶¨
        start_sample = trak->start_sample + 1;
        ...
        //end_sampleÔÚngx_http_mp4_update_stts_atomÖÐµÄngx_http_mp4_crop_stts_dataÖÐ±»È·¶¨
    ...
    ngx_http_mp4_trak_t *trak, ngx_uint_t start)
{
    ...
    if (start) {
		// start_sample ÔÚngx_http_mp4_crop_stts_dataÖÐ¸üÐÂ
        ...
        // end_sampleÔÚngx_http_mp4_crop_stts_dataÖÐ¸üÐÂ
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (data) {
        ...
        // trak->start_sampleºÍtrak.end_sample¾ùÔÚngx_http_mp4_crop_stts_dataÖÐ¸üÐÂ
